<h3 align="center">Backend Developer | AI Engineer </h3>

---

<p align="center">

<p align="center">
  <em>
    Apasionado por la tecnología, busco innovar y generar soluciones que resuelvan problemas cotidianos. Emocionado por conocer hasta que punto puede llegar la IA en el futuro.
  </em>
  <br><br>
<b><i align="center">"¿Qué lugar ocuparemos cuando existan sistemas que hacen casi todo mejor?”</i></b><br>
<b><i align="center">Filosofía: "Fundamentos y Conceptos  >>  Código”</i></b>
</p>
<br><br>
<img align='right' src='https://github.com/AntonioCebada/AntonioCebada/blob/main/media/octocat-anime.gif?raw=true' width='200'>

<img src="https://media.giphy.com/media/iY8CRBdQXODJSCERIr/giphy.gif" width="30px">&nbsp;**_About me ...._**

✔ Me enfoco principalmente en el lado del **servidor**, deploy y cloud services.<br>
✔ He desarrollado sitios web que automatizan procesos incrementando eficiencia, reduciendo coste y mejorando la UX.<br>
✔ Planeo, diseño y hasta entonces tiro código.<br>
✔ Uso Spec Driven Development (**SDD**) como metodologia de desarrollo.<br>
✔ Mi entorno de desarrollo es 100% linux.<br><br>

<img src="https://media.giphy.com/media/iY8CRBdQXODJSCERIr/giphy.gif" width="30px">&nbsp;**_My working tools..._**

```
Frontend      Vue · TypeScript · Tailwind
Backend       Laravel · PHP
Databases     MySQL/MariaDB · SQLite
AI Tooling    MCP Servers · OpenCode
Terminal      Bash · LazyVim · Tmux
Cloud         Dokploy · n8n
```

<br>

<img src="https://media.giphy.com/media/iY8CRBdQXODJSCERIr/giphy.gif" width="30px">&nbsp;**_Github Stats ...._**

![](https://github-readme-stats.shion.dev/api?username=AntonioCebada&theme=tokyonight&hide_border=false&include_all_commits=false&count_private=false)
